...
class PeterLynchAlphaModel(PersonaAlphaModel):
    def __init__(self):
    ...
    def update(self, state: AgentState, data: str) -> list[Insight]:
        ...
        progress.update_status(self.agent_id, ticker, "Strolling Main Street")
        ticker_data = quant_scorecard.get(ticker, {})
        metrics = ticker_data.get("metrics", {})
        facts = {"metrics": metrics, "rules": factor_rules}

        template = ChatPromptTemplate.from_messages([
            ("system", "You are Peter Lynch. Look for ten-baggers. YOUR RULES: {rules}. Output JSON only."),
            ("human", "Ticker: {ticker}\nScorecard: {facts}")
        ])
...
def peter_lynch_agent(state: AgentState, agent_id: str = "peter_lynch"):
    model = PeterLynchAlphaModel()
    insights = []
    ...
    return {"messages": [HumanMessage(content="Analysis complete", name="peter_lynch")], "data": state["data"]}
