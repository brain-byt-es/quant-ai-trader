...
class StanleyDruckenmillerAlphaModel(PersonaAlphaModel):
    def __init__(self):
        super().__init__("stanley_druckenmiller")
    ...
    def update(self, state: AgentState, ticker: str) -> list[Insight]:
        ...
        progress.update_status(self.agent_id, ticker, "Macro Audit")
        ticker_data = quant_scorecard.get(ticker, {})
        metrics = ticker_data.get("metrics", {})
        facts = {"metrics": metrics, "rules": factor_rules}

        template = ChatPromptTemplate.from_messages([
            ("system", "You are Stanley Druckenmiller. Evaluate the ticker based on macro and momentum. YOUR RULES: {rules}. Output JSON only."),
            ("human", "Ticker: {ticker}\nScorecard: {facts}")
        ])
...
def stanley_druckenmiller_agent(state: AgentState, agent_id: str = "stanley_druckenmiller"):
    model = StanleyDruckenmillerAlphaModel()
    insights = []
    ...
    return {"messages": [HumanMessage(content="Analysis complete", name="stanley_druckenmiller")], "data": state["data"]}
