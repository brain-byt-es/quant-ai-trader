...
class BillAckmanAlphaModel(PersonaAlphaModel):
    def __init__(self):
        super().__init__("bill_ackman")
    ...
    def update(self, state: AgentState, data: str) -> list[Insight]:
        ...
        progress.update_status(self.agent_id, ticker, "Activist Review")
        ticker_data = quant_scorecard.get(ticker, {})
        metrics = ticker_data.get("metrics", {})
        facts = {"metrics": metrics, "rules": factor_rules}

        template = ChatPromptTemplate.from_messages([
            ("system", "You are Bill Ackman. Be precise and activist. YOUR RULES: {rules}. Output JSON only."),
            ("human", "Ticker: {ticker}\nScorecard: {facts}")
        ])
...
def bill_ackman_agent(state: AgentState, agent_id: str = "bill_ackman"):
    model = BillAckmanAlphaModel()
    insights = []
    ...
    return {"messages": [HumanMessage(content="Analysis complete", name="bill_ackman")], "data": state["data"]}
