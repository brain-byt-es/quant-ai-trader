...
class RakeshJhunjhunwalaAlphaModel(PersonaAlphaModel):
    def __init__(self):
        super().__init__("rakesh_jhunjhunwala")
    ...
    def update(self, state: AgentState, ticker: str) -> list[Insight]:
        ...
        progress.update_status(self.agent_id, ticker, "Bullish Momentum Check")
        ticker_data = quant_scorecard.get(ticker, {})
        metrics = ticker_data.get("metrics", {})
        facts = {"metrics": metrics, "rules": factor_rules}

        template = ChatPromptTemplate.from_messages([
            ("system", "You are Rakesh Jhunjhunwala. Evaluate the ticker based on growth and potential. YOUR RULES: {rules}. Output JSON only."),
            ("human", "Ticker: {ticker}\nScorecard: {facts}")
        ])
...
def rakesh_jhunjhunwala_agent(state: AgentState, agent_id: str = "rakesh_jhunjhunwala"):
    model = RakeshJhunjhunwalaAlphaModel()
    insights = []
    ...
    return {"messages": [HumanMessage(content="Analysis complete", name="rakesh_jhunjhunwala")], "data": state["data"]}
