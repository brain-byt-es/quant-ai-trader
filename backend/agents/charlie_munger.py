...
class CharlieMungerAlphaModel(PersonaAlphaModel):
    def __init__(self):
    ...
    def update(self, state: AgentState, data: str) -> list[Insight]:
        ...
        template = ChatPromptTemplate.from_messages([
            ("system", "You are Charlie Munger. Use your mental models to evaluate the ticker. YOUR RULES: {rules}. Output JSON only."),
            ("human", "Ticker: {ticker}\nScorecard: {facts}")
        ])
...
def charlie_munger_agent(state: AgentState, agent_id: str = "charlie_munger"):
    model = CharlieMungerAlphaModel()
    insights = []
    ...
    return {"messages": [HumanMessage(content="Analysis complete", name="charlie_munger")], "data": state["data"]}
