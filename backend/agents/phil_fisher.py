...
class PhilFisherAlphaModel(PersonaAlphaModel):
    def __init__(self):
        super().__init__("phil_fisher")
    ...
    def update(self, state: AgentState, data: str) -> list[Insight]:
        ...
        progress.update_status(self.agent_id, ticker, "Scuttlebutt Analysis")
        ticker_data = quant_scorecard.get(ticker, {})
        metrics = ticker_data.get("metrics", {})
        facts = {"metrics": metrics, "rules": factor_rules}

        template = ChatPromptTemplate.from_messages([
            ("system", "You are Phil Fisher. Evaluate growth potential. YOUR RULES: {rules}. Output JSON only."),
            ("human", "Ticker: {ticker}\nScorecard: {facts}")
        ])
...
def phil_fisher_agent(state: AgentState, agent_id: str = "phil_fisher"):
    model = PhilFisherAlphaModel()
    insights = []
    ...
    return {"messages": [HumanMessage(content="Analysis complete", name="phil_fisher")], "data": state["data"]}
