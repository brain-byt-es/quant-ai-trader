...
class MohnishPabraiAlphaModel(PersonaAlphaModel):
    def __init__(self):
        super().__init__("mohnish_pabrai")
    ...
    def update(self, state: AgentState, ticker: str) -> list[Insight]:
        ...
        progress.update_status(self.agent_id, ticker, "Checklist Review")
        ticker_data = quant_scorecard.get(ticker, {})
        metrics = ticker_data.get("metrics", {})
        facts = {"metrics": metrics, "rules": factor_rules}

        template = ChatPromptTemplate.from_messages([
            ("system", "You are Mohnish Pabrai. Use your checklist. YOUR RULES: {rules}. Output JSON only."),
            ("human", "Ticker: {ticker}\nScorecard: {facts}")
        ])
...
def mohnish_pabrai_agent(state: AgentState, agent_id: str = "mohnish_pabrai"):
    model = MohnishPabraiAlphaModel()
    insights = []
    ...
    return {"messages": [HumanMessage(content="Analysis complete", name="mohnish_pabrai")], "data": state["data"]}
