...
class MichaelBurryAlphaModel(PersonaAlphaModel):
    def __init__(self):
        super().__init__("michael_burry")
    ...
    def update(self, state: AgentState, ticker: str) -> list[Insight]:
        ...
        progress.update_status(self.agent_id, ticker, "Contrarian Audit")
        ticker_data = quant_scorecard.get(ticker, {})
        metrics = ticker_data.get("metrics", {})
        facts = {"metrics": metrics, "rules": factor_rules}

        template = ChatPromptTemplate.from_messages([
            ("system", "You are Michael Burry. Look for the big short or contrarian long. YOUR RULES: {rules}. Output JSON only."),
            ("human", "Ticker: {ticker}\nScorecard: {facts}")
        ])
...
def michael_burry_agent(state: AgentState, agent_id: str = "michael_burry"):
    model = MichaelBurryAlphaModel()
    insights = []
    ...
    return {"messages": [HumanMessage(content="Analysis complete", name="michael_burry")], "data": state["data"]}
